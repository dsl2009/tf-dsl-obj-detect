...
def train():
    ...
    sv = tf.train.Supervisor(logdir='/home/dsl/all_check/face_detect/nana', summary_op=None, init_fn=restore)
...
def detect():
    ...
    with tf.Session() as sess:
        ...
        for ip in glob.glob('/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/face_detect/originalPics/2002/07/19/big/*.jpg'):
            ...
            org, window, scale, padding, crop = utils.resize_image(img, min_dim=448, max_dim=448)
            ...
            visual.display_instances_title(org,np.asarray(bxx)*448,class_ids=np.asarray(cls),class_names=['face'],scores=scores)
